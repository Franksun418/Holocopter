...
public class ObjectHelper {
	...
	public void Draw(float width) {
		...
		if (ObjectMerge.ObjectDrawCheck()) {
			...
			if (GUILayout.Button(show ? "  -   " : "  +   ", GUILayout.Width(19))) {	 //Positioning on this button is super screwy
				show = !show;
	...
	GameObject Copy(GameObject original, bool isMine) {
		...
		if(UniMerge.Util.IsPrefabParent(original) && PrefabUtility.GetPrefabType(original) == PrefabType.PrefabInstance) {
			copy = (GameObject)PrefabUtility.InstantiatePrefab(PrefabUtility.GetCorrespondingObjectFromSource(original));
#endif
...
#region ATTRIBUTES
	...
	void FindAndSetRefs(GameObject source, GameObject copy, bool isMine) {
		...
		for(int i = 0; i < sourceList.Count; i++) {
			...
			for(int j = 0; j < sourceComps.Length; j++) {
				...
				ObjectMerge.root.UnsetFlagRecursive();
				List<SerializedProperty> srcProps = new List<SerializedProperty>();
				List<SerializedProperty> copyProps = new List<SerializedProperty>();
				ComponentHelper.GetProperties(srcProps, new SerializedObject(sourceComps[j]));
				ComponentHelper.GetProperties(copyProps, new SerializedObject(copyComps[j]));
				for(int k = 0; k < srcProps.Count; k++) {
					if(srcProps[k].name == "m_Script")	//Ignore the script
						continue;	
					if(srcProps[k].propertyType == SerializedPropertyType.ObjectReference && srcProps[k].objectReferenceValue != null) {
						if(srcProps[k].objectReferenceValue == sourceList[i])
							continue;
						if(srcProps[k].objectReferenceValue.GetType() == typeof(GameObject)) {
							ObjectHelper other = FindObj(srcProps[k].objectReferenceValue, isMine);
							if(other != null) {
								copyProps[k].objectReferenceValue = other.GetObject(!isMine);
							}
						} else {
							ComponentHelper other = FindComp(srcProps[k].objectReferenceValue, isMine);
							if(other != null) {
								copyProps[k].objectReferenceValue = other.GetComponent(!isMine);
							}
						}
						if(copyProps[k].serializedObject.targetObject != null)
							copyProps[k].serializedObject.ApplyModifiedProperties();
						ObjectMerge.root.UnsetFlagRecursive();
					}
				}
			}
	...
	public ObjectHelper FindObj(Object find, bool isMine) {
		if(findAndSetFlag)
			return null;
		findAndSetFlag = true;
		if(GetObject(isMine) == find)
			return this; 
		foreach(ObjectHelper helper in children) {
			ObjectHelper ohelp = helper.FindObj(find, isMine);
			if(ohelp != null)
				return ohelp;
		}
		if(parent != null) {
			return parent.FindObj(find, isMine);
		}
		return null;
	}
	public ComponentHelper FindComp(Object find, bool isMine) {
		if(findAndSetFlag)
			return null;
		findAndSetFlag = true;
		foreach(ComponentHelper comp in components) {
			if(comp.GetComponent(isMine) == find)
				return comp;
		}
		foreach(ObjectHelper helper in children) {
			ComponentHelper ohelp = helper.FindComp(find, isMine);
			if(ohelp != null)
				return ohelp;
		}
		if(parent != null) {
			return parent.FindComp(find, isMine);
		}
		return null;
	}
	/// <summary>
	...
	private void DrawObject(bool isMine, float width) {
		...
		UniMerge.Util.Indent(width, delegate {
			...
			if(GetObject(isMine)) {
				...
				if(GUILayout.Button(new GUIContent("P", "Ping the object in the heirarchy")))
					EditorGUIUtility.PingObject(GetObject(isMine));
